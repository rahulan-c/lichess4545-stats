...
Standings that try to account for the results of games that have been played in current rounds. They might not always be consistent with the official standings published on the Lichess4545 website because they don't properly include every tiebreak measure.
...
### 4545 Team League - Season `r season`

*Last updated: `r lubridate::now("UTC")`*
...
# Add opposition name to data
# scores2 <- dplyr::left_join(scores2, )
...
scores2 <- scores2 %>% 
  ...
  select(rank, team, match_pts, game_pts, opp_rank, live_score, games_left, change_rank, hypo_rank) %>% 
  arrange(rank)
...
reactable(scores2, 
          ...
          columns = list(
            ...
            live_score = colDef(name = "Current score", align = "center", minWidth = 120),
            games_left = colDef(name = "Games left", align = "center", minWidth = 70),
            
            change_rank = colDef(name = "+/-", align = "center", minWidth = 70),
            hypo_rank = colDef(name = "New rank", align = "center", minWidth = 70)
            
          ),
          columnGroups = list(
            colGroup(name = paste0("Current standings after R", round - 1), columns = c("rank", "team", "match_pts", "game_pts")),
            colGroup(name = paste0("Current round (R", round, ")"), columns = c("opp_rank", "live_score", "games_left")),
