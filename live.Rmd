...
For S`r season` R`r round` (last updated: `r Sys.Date()`) 
...
scores2 <- scores2 %>% 
  ...
  mutate(live_margin = glue::glue("{ifelse(score > opp_score, paste0(score - opp_score, ' pts ahead'), 
                                  ifelse(score < opp_score, paste0(opp_score - score, ' pts behind'), 'Tied'))}, {boards - (score + opp_score)} games left"))
