...
if(league == "team4545"){knitr::include_graphics("img/all-for-one.svg")}
if(league == "lonewolf"){knitr::include_graphics("img/wolf-head.svg")}
```
...
if(league == "team4545"){
  ...
  first <- podium_places[[1]]
  first_link <- podium_places[[2]]
  first_score <- podium_places[[3]]
  first_perf <- podium_places[[4]]
  
  second <- podium_places[[5]]
  second_link <- podium_places[[6]]
  second_score <- podium_places[[7]]
  second_perf <- podium_places[[8]]
  
  third <- podium_places[[9]]
  third_link <- podium_places[[10]]
  third_score <- podium_places[[11]]
  third_perf <- podium_places[[12]]
...
See the twists and turns in every match this season through these PDF visualisations...assuming they've already been created and uploaded. [Source script](https://github.com/rahulan-c/lichess4545-stats/blob/main/scripts/plot_match_story.R).
...
### Top players `r emo::ji("tophat")`


#### Board medals

Here are the top player performances on each board (minimum 5 games played).^[Note that these performance ratings only account for account games played on a specific board, so they differ in this respect from the performance ratings published on the Lichess4545 rosters page.]
          ...
          columns = list(
            Image = colDef(name = "Award", 
                           cell = function(value, index) {
                             ...
                             image <- img(src = sprintf("img/%s.svg", award_name), height = "60px", alt = award_name)
              tagList(
...
if(notable_mates != ""){
  knitr::include_graphics(paste0("img/mates/",
