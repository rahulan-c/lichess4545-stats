...
description: |
  Possibly the most requested thing ever...   
output:
...
resize_image <- function(image) {
  ...
  magick::image_write(imFile_resized, paste0(img_path, image))
}
...
make_gallery_layout <- function() {
  ...
  tagList(apply(images, 1, function(x) {
      tags$a(
        href = paste0(img_path, x[["images_full_size"]]),
        tags$img(src = paste0(img_path, x[["images_thumb"]]))
      )
  }))
