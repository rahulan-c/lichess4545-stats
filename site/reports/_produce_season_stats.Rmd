...
if(league == "team4545"){
  ...
  first <- podium_places[[1]]
  first_link <- podium_places[[2]]
  first_score <- podium_places[[3]] %>% str_replace("Score: ", "")
  first_perf <- podium_places[[4]]
  
  second <- podium_places[[5]]
  second_link <- podium_places[[6]]
  second_score <- podium_places[[7]] %>% str_replace("Score: ", "")
  second_perf <- podium_places[[8]]
  
  third <- podium_places[[9]]
  third_link <- podium_places[[10]]
  third_score <- podium_places[[11]] %>% str_replace("Score: ", "")
  third_perf <- podium_places[[12]]
  
  prize <- podium_places[[13]]
  prize_link <- podium_places[[14]]
  prize_score <- podium_places[[15]] %>% str_replace("Score: ", "")
  prize_perf <- podium_places[[16]]
...
<p style="font-size:11pt; font-weight: 600;">
  `r paste0(prize_score, " (", prize_perf, " perf)")`
</p>
:::
...
reactable(bperfs, groupBy = "board",
          ...
          columns = list(
            board = colDef(name = "Board",
                          align = "center",
                          minWidth = 100),
            rank = colDef(cell = function(value) {
              image <- img(src = sprintf("report_img/%s.png", value), style = "height: 24px;", alt = value)
              tagList(
