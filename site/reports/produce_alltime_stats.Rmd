...
games <- readRDS("C:/Users/rahul/Documents/Github/rahulan-c.github.io/lichess4545-stats/data/combined_league_games_210804_tidied.rds")
...
season_dates <- paste0(lubridate::day(min(games_incl_banned$started)), " ",
       lubridate::month(min(games_incl_banned$started), label = T), " ", 
       lubridate::year(min(games_incl_banned$started)),
       " to ",
       lubridate::day(max(games_incl_banned$started)), " ",
       lubridate::month(max(games_incl_banned$started), label = T), " ",
       lubridate::year(max(games_incl_banned$started)))
...
if(league == "team4545"){
  ...
  games$rating_group <- cut(games$mean_rating, 
                                breaks=c(0, 1200, 1400, 1600, 1800, 2000,
                                         2200, 4000),
                                labels = c("< 1200",
...
if(league == "lonewolf"){
  ...
  position_tracker <- positions %>% 
    ...
    scale_color_manual(values = c("#ffd700", "#7b7b7b", "#cd7f32", rep("#d1ebe7", 7)),
                       breaks = c(top_places),
                       labels = c(top_places)) +
    theme(panel.grid.minor = element_blank()) +
    theme(
...
plt_gamesbyday <- games_by_day_time %>% 
  ...
  ggplot(aes(wday, n)) +
  geom_col(fill = league_col, colour = league_col) + # league_col
...
heat_dayhour <- ggplot(games_by_day_time, aes(hour, wday)) + geom_tile(aes(fill = N),colour = "white", na.rm = TRUE) +
  scale_fill_gradient(low = "white", high = league_col) + # league_col
...
season_think <- rbind(tibble("player" = games$white, "duration" = games$duration_w, "moves" = games$num_moves),
  ...
  mutate(duration_print = sprintf('%2dd %2dh %2dm %2ds', lubridate::day(total_duration), lubridate::hour(total_duration), lubridate::minute(total_duration), lubridate::second(total_duration))) %>% 
...
most_playing_time <- rbind(tibble("player" = games$white, "duration" = games$duration),
  ...
  arrange(desc(total_duration)) %>% 
  mutate(rank = dense_rank(desc(total_duration))) %>% 
  mutate(duration_print = sprintf('%2gd %2gh %2gm %2.0fs', lubridate::day(total_duration), lubridate::hour(total_duration), lubridate::minute(total_duration), lubridate::second(total_duration))) %>% 
  select(rank, player, games, duration_print) %>% 
  head(50)
```
...
movetimes <- left_join(movetimes, games_basics, by = c("game_id" = "id")) %>% 
  ...
  mutate(time_spent = sprintf('%2dh %2dm %2ds', lubridate::hour(time_spent), lubridate::minute(time_spent), lubridate::second(time_spent))) %>% 
