...
cat(token) # DEBUGGING - DELETE LATER
...
print(paste0(here::here())) # DEBUGGING - DELETE LATER
...
for(l in seq(1:length(all_player_ids))){
  batch_ids <- all_player_ids[[l]] %>% str_c(collapse = ",")
  profiles <- httr::POST(
  ...
  profiles <- profiles %>% 
    httr::content("text", encoding = stringi::stri_enc_detect(content(profiles, "raw"))[[1]][1,1]) %>% 
    jsonlite::fromJSON()
...
players_by_country <- players_by_country %>% 
  filter(!(country %in% c("_lichess", "_rainbow", "_pirate", "_earth", 
                          "_united-nations", "AQ"))) %>% 
  filter(!(is.na(country))) %>% 
  tibble::as_tibble()
