---
title: "`r paste0(ifelse(league == 'lonewolf', 'LoneWolf', '4545'), ifelse(lw_section == 'open', ' (Open)', ifelse(lw_section == 'u1800', ' (U1800)', '')), ' Season ', s)`"
description: |
...
# Use xaringanExtra's panelset for tabs that work with distill
...
gambits <- read.csv(glue::glue("{here::here()}/data/lookup/gambits.csv"))
# N.B. THIS IS INCOMPLETE DATA AND NEEDS TO BE REVISED
...
# Eg league, season, rounds, section etc.
# league <- league
# season <- s
...
# Load data on all 4545 or LW games - required for determing the Rookie Award
if(league == "team4545") {
  all_games <- readr::read_rds(paste0(data_path, "allgames_team.rds")) %>% 
    as_tibble()
} else if (league == "lonewolf") {
  all_games <- readr::read_rds(paste0(data_path, "allgames_lw.rds")) %>% 
    as_tibble()
}
...
```{r, fig.align='center', out.width = "15%", out.height = "15%"}
...
- The `r paste0("[season]", "(", summary_url, ")")` was played from `r season_dates`.
- `r formatC(length(unique(c(unique(website_pairings$white), unique(website_pairings$black)))), big.mark = ",")` players took part, playing `r formatC(nrow(website_pairings), big.mark = ",")` games in total. The season's forfeit rate was `r round(((nrow(pairings) - nrow(website_pairings)) / nrow(pairings)) * 100, 1)` per cent.^[The forfeit rate is the percentage of pairings that didn't result in a game being played.]
- White won `r formatC(games_w, big.mark=",")` games (`r round(perc_w*100, 1)`%), `r formatC(games_d, big.mark=",")` games (`r round(perc_d*100, 1)`%) were drawn, and Black won `r formatC(games_b, big.mark=",")` games (`r round(perc_b*100, 1)`%).^[The number of games played in this statistic can differ from the number of games played in the season, as it excludes games where the result of the pairing was later changed to a forfeit win or loss.]
...
```{r, eval = ifelse(league == "team4545", TRUE, FALSE), layout="l-page"}
...
```{r position_tracker, out.width="100%", fig.align="center"}
...
The top player performances by board (minimum 5 games played).^[The performance ratings in this table only take into account games played on the relevant board, which is why they differ from (and should be preferred to) the ratings given by the board standings tables on the Lichess4545 website.]
...
#### When were games played? {.panel-name}

```{r games_by_day, fig.show="hold", out.width = "50%", layout="l-body-outset"}
...
#### How long did games take? {.panel-name}

```{r hists, fig.show="hold", out.width = "50%", layout="l-body-outset"}
...
#### Longest games (moves) {.panel-name}
...
```{r longest_moves, layout="l-body-outset"}
...
```{r longest_time, layout="l-body-outset"}
...
::::: {.panelset}
::: {.panel}
#### ACPL / blunder rate {.panel-name}

```{r acpl_by_board, fig.show="hold", out.width = "50%", eval = ifelse(league == "team4545", TRUE, FALSE)}
...
```{r error_rates_by_rating_band, fig.show="hold", out.width = "50%", eval = ifelse(league == "team4545", FALSE, TRUE)}
...
:::
::: {.panel}
#### Games with lowest combined ACPL {.panel-name}
...
:::
::: {.panel}
#### Best for Black {.panel-name}

The best-scoring main openings for Black. 

*Includes all main openings played in at least 10 games.* 

```{r openings_for_black, layout="l-page"}
...
:::
::: {.panel}
#### Popular gambits {.panel-name}

All gambit openings played in the season.^[Currently, gambit openings are identified by the presence of the term *gambit* in the opening name assigned by Lichess. This is admittedly an imperfect approach as it excludes certain openings that should also be considered as gambits such as the Marshall Attack.]
...
reactable(gambiteers,
    ...
    games = colDef(name = "Games", align = "center"),
    success = colDef(name = "% Score", format = colFormat(digits = 0), align = "center")
           )
    )
} else {
...
rookie_perfs <- perfs %>% 
  ...
  filter(!(str_to_lower(player) %in% str_to_lower(tos_violators))) %>% # remove ToS violators
  filter(games >= 4) %>% # apply lower games played threshold for rookie award
  filter(perf_diff >= 0) %>% 
  arrange(desc(perf_diff)) %>% 
  mutate(perf_rank = dense_rank(desc(perf_diff))) %>% 
  select(perf_rank, player, games, wins, draws, losses, initial_rating, perf_rating, perf_diff)
...
```{r player_awards_4545, eval = ifelse(league == "team4545", TRUE, FALSE), layout="l-page"}
...
awards <- tibble("Image" = c(rep("x", 20)),
                 "Award" = c("Gambit Guru",
                             "MVP Award",
                             ...
                             ifelse(league == "team4545", "Egalitarian Award", ""),
                             "Rookie Award"
                             ),
                 ...
                 "Definition" = c("Best score playing gambits",
                                  ...
                                  ifelse(league == "team4545", "Team with the lowest SD across players' relative perfs", ""),
                 ...
                 "Winner" = c(gambiteers$player[1],
                              ...
                              ifelse(league == "team4545", egalitarian$team[1], ""),
                              ifelse(nrow(rookie_perfs) > 0, rookie_perfs$player[1], "")
                              ),
                 
                 
                 "Details" = c(ifelse(gambiteer_award, paste0(round(gambiteers$success[1]), "% from ", gambiteers$games[1], " games"), "Can't be awarded"),
                              ...
                              ifelse(nrow(minacpl_players) > 0, paste0(minacpl_players$n[1], " games"), ""),
                             ...
                             ifelse(league == "team4545", paste0("SD: ", round(egalitarian$egalite[1], 1)), ""),
...
reactable(awards,
          ...
          columns = list(
            ...
            Winner = colDef(name = "Winner", minWidth = 170,
                            style = list(color = "#245164", #245164
                                         fontWeight = "bold")),
...
```{r player_awards_lw, eval = ifelse(league == "team4545", FALSE, TRUE), layout="l-page"}

single_fig_acpl_players <- minacpl_players %>% select(player) %>% dplyr::pull()
giri_winners <- drawers %>% select(player) %>% dplyr::pull()
giri_value <- drawers %>% select(n) %>% distinct() %>% dplyr::pull()

awards <- tibble("Image" = c(rep("x", 17)),
                 "Award" = c("Gambit Guru",
                             "MVP Award",
                             "Archbishop of Accuracy",
                             "Primates of Precision",
...
```{r team_stats_appendix, eval = ifelse(league == "team4545", TRUE, FALSE), layout="l-page"}
...
::::: {.panelset}
::: {.panel} 
##### Best relative perfs {.panel-name}
...
:::
::: {.panel}
##### Time spent per move (after move 10) {.panel-name}
...
:::
::: {.panel} 
##### Strength of schedule {.panel-name}
...
::::: {.panelset}
::: {.panel}
##### Least time left after move 10 {.panel-name}
...
:::
::: {.panel}
##### Time trouble addicts {.panel-name}
...
::::: {.panelset}
::: {.panel}
##### Lesser spotted mates {.panel-name}
...
:::
::: {.panel}
##### Rollercoaster games {.panel-name}
...
:::
::: {.panel}
##### Checks and captures {.panel-name}
