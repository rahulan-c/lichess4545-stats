...
output:
  html_document:
    ...
    toc_depth: 4
    toc_float: true
title: "`r paste0(ifelse(league == 'lonewolf', 'LoneWolf', '4545 League'), ifelse(lw_section == 'open', ' (Open)', ifelse(lw_section == 'u1800', ' (U1800)', '')), ', Season ', s)`"
---
...
awards <- tibble("Image" = c(rep("x", 20)),
                 ...
                 "Definition" = c("Best score playing gambits",
                                  ...
                                  ifelse(league == "team4545", "Lowest standard deviation across players' relative performance ratings", ""),
...
- These statistics are presented for general interest only. They are not guaranteed to be comprehensive, complete^[For example, the games data used for this report excludes games of less than 5 plies (half-moves) as well as games that ended in disconnection (ie where one player claimed a win or draw after their opponent lost connection, which isn't allowed in the leagues) or cheat detection, or where the game was aborted.], accurate, or up to date. They may be revised periodically to incorporate minor updates and bugfixes.
- Any player determined by Lichess to have violated their Terms of Service (as of `r format(Sys.time(), '%d %B %Y')`) has been excluded from the player-based statistics and rankings shown here.^[Players who were banned from the leagues after finishing a season in the top three places are still shown with a podium place in that season's report.]
