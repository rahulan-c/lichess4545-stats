...
# Get team with the most close wins
close_wins <- close %>% 
  group_by(winner) %>% 
...
Check out these visualisations of every match in every round!^[These require the original match story PDFs for the round to have previously been produced, so links for older seasons may not work (until I've produced them for all seasons).] 
...
Congratulations to all the **top scorers** this season! Here's everyone who achieved a plus score of +2.5 or higher:

`r top_scorers_text`

Everyone who scored +3 or more gets the **Ace** award.
:::
...
### Stats - openings {.tabset .tabset-fade .tabset-pills} 
...
### Highlights `r emo::ji("television")`

```{r}
highlight_ids <- "RbeDVNpd"

highlights <- c("<iframe src=",
                  paste0("https://lichess.org/embed/", highlight_ids, "?theme=auto&bg=auto"),
                  ">")
```

```{=html}
<iframe src="https://lichess.org/embed/RbeDVNpd#49?theme=auto&bg=auto"
width="600" height="397" frameborder=0 allowfullscreen="" data-external="1"></iframe>
```
...
*Includes everyone with a season ACPL ranked in the 20th percentile and above who played at least five games.*
...
The players who faced the best-scoring opponents over the season, when judged by their collective score over all their *other* games. *Originally proposed by Tranzoo.*
...
The players with the least time left after 10 moves...who still managed to win! *Originally proposed by Silkthewanderer.*
